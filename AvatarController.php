...
class AvatarController extends Controller {
	...
	public function getAvatarDark(string $userId, int $size, bool $guestFallback = false) {
		...
		try {
			...
			$this->logger->error('Error fetching dark avatar: ' . $e->getMessage(), ['exception' => $e]);
	...
	public function getAvatar(string $userId, int $size, bool $guestFallback = false) {
		...
		try {
			$avatar = $this->avatarManager->getAvatar($userId);
			// Pass false for regular mode (default)
			$avatarFile = $avatar->getFile($size, false);
			$response = new FileDisplayResponse(
				$avatarFile,
				Http::STATUS_OK,
				['Content-Type' => $avatarFile->getMimeType(), 'X-NC-IsCustomAvatar' => (int)$avatar->isCustomAvatar()]
			...
			$this->logger->error('Error fetching avatar: ' . $e->getMessage(), ['exception' => $e]);
			return new JSONResponse([], Http::STATUS_NOT_FOUND);
		}
	...
	public function postAvatar(?string $path = null): JSONResponse {
		// Disable avatar upload for external avatar system
		return new JSONResponse(
			['data' => ['message' => $this->l10n->t('Avatar upload is disabled. Avatars are fetched from external system.')]],
	...
	public function deleteAvatar(): JSONResponse {
		try {
			$avatar = $this->avatarManager->getAvatar($this->userId);
			// Only clear cache, don't actually delete from external system
			$avatar->remove();
			return new JSONResponse();
		} catch (\Exception $e) {
			$this->logger->error($e->getMessage(), ['exception' => $e, 'app' => 'core']);
